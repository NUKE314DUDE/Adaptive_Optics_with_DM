...
def smoothed_sawtooth(fill = 0.95, cut_freq_low = None, cut_freq_high = None, sig_freq = None, dm_sample_duration = None):
    ...
    padding_size = 2*len(defocus_seq)
    defocus_seq = np.pad(defocus_seq, padding_size, mode = 'wrap')
    if cut_freq_low is None and cut_freq_high is None:
        out_put = normalization(defocus_seq)
        return out_put[padding_size:-padding_size]
    else:
        defocus_seq = pass_filter(defocus_seq, 1 / dm_sample_duration, cut_freq_low, cut_freq_high)
        diff_defocus_seq = np.diff(defocus_seq)
        vally_index = np.where((diff_defocus_seq[:-1] < 0) & (diff_defocus_seq[1:] > 0))[0]+1
        out_put = normalization(defocus_seq[vally_index[1]:vally_index[2]])
        return out_put
